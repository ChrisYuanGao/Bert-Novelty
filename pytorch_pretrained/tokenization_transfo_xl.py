...
class TransfoXLCorpus(object):
    ...
    def build_corpus(self, path, dataset):
        ...
        if self.dataset in ['ptb', 'wt2', 'enwik8', 'text8']:
            ...
            self.vocab.count_file(os.path.join(path,'predict.txt'))
        ...
        if self.dataset in ['ptb', 'wt2', 'wt103']:
            ...
            self.predict = self.vocab.encode_file(
                os.path.join(path, 'predict.txt'), ordered=True)
            ...
            self.predict = self.vocab.encode_file(
                os.path.join(path, 'predict.txt'), ordered=True, add_eos=False)
        elif self.dataset == 'lm1b':
    ...
    def get_iterator(self, split, *args, **kwargs):
        if split == 'train':
            ...
            if self.dataset in ['ptb', 'wt2', 'wt103', 'enwik8', 'text8']:
                to_pred = LMOrderedIterator(data, *args, **kwargs)
            elif self.dataset == 'lm1b':
                to_pred = LMShuffledIterator(data, *args, **kwargs)

        return data_iter,to_pred
